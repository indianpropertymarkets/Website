 ...
 * @param {string} phone - Phone number (digits only, with country code)
 * @param {string} [message=''] - Optional pre-filled message
 */
export function chatOnWhatsApp(phone, message = '') {
...
/**
 * Generates a WhatsApp link element
 * @param {string} phone
 * @param {string} label
 * @returns {string} HTML string
 */
export function whatsappButtonHTML(phone, label = 'WhatsApp') {
    const cleanPhone = phone.replace(/[^0-9]/g, '');
    const fullPhone = cleanPhone.startsWith('91') ? cleanPhone : `91${cleanPhone}`;
    return `<a href="https://wa.me/${fullPhone}" target="_blank" class="whatsapp-btn-sm">
